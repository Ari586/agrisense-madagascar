...
export async function GET(): Promise<NextResponse<AlertsData | { error: string }>> {
  try {
    ...
    if (highRes.ok) {
        ...
        if (minTemp < 10) {
            alerts.push({ id: "high-cold", type: "rain", severity: "warning", title: "Hatsiaka (Fanala)", message: `Hatsiaka be (${minTemp}°C) any Vakinankaratra. Arovy ny voly saropady.`, region: "Hauts Plateaux", time: "Mivantana" });
        }
        if (rain > 15) {
